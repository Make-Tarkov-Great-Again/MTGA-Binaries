...
using MTGAConstant = MTGA.Core.PatchConstants;

namespace MTGA.Utilities.Player
{
    public class HealthListener
    {
        ...
        private readonly MTGA.Core.Request _request;
        ...
        {
            _request = new MTGA.Core.Request(MTGAConstant.GetPHPSESSID(), MTGAConstant.GetBackendUrl());
        ...
        private void SetCurrent(string v)
        {
            ...
            if (MTGAConstant.GetAllPropertiesForObject(MyHealthController).Any(x => x.Name == v)) 
            {
                var valuestruct = MTGAConstant.GetAllPropertiesForObject(MyHealthController).FirstOrDefault(x => x.Name == v).GetValue(MyHealthController);
                if (valuestruct == null)
                    return;

                var currentEnergy = MTGAConstant.GetAllFieldsForObject(valuestruct).FirstOrDefault(x => x.Name == "Current").GetValue(valuestruct);
            ...
            else if (MTGAConstant.GetAllFieldsForObject(MyHealthController).Any(x => x.Name == v))
            {
                var valuestruct = MTGAConstant.GetAllFieldsForObject(MyHealthController).FirstOrDefault(x => x.Name == v).GetValue(MyHealthController);
                if (valuestruct == null)
                    return;
        ...
        private void SetCurrentHealth(object healthController, IReadOnlyDictionary<EBodyPart, BodyPartHealth> dictionary, EBodyPart bodyPart)
        {
            ...
            {
                MTGAConstant.Logger.LogInfo("HealthListener:GetBodyPartHealth:HealthController is NULL");
                return;
            }
            ...
            {
                MTGAConstant.Logger.LogInfo("HealthListener:GetBodyPartHealth not found!");
                return;
            }
            ...
            var current = MTGAConstant.GetAllFieldsForObject(bodyPartHealth).FirstOrDefault(x => x.Name == "Current").GetValue(bodyPartHealth).ToString();
            var maximum = MTGAConstant.GetAllFieldsForObject(bodyPartHealth).FirstOrDefault(x => x.Name == "Maximum").GetValue(bodyPartHealth).ToString();
            //var maximum = PatchConstants.GetFieldOrPropertyFromInstance<float>(bodyPartHealth, "Maximum", true);
